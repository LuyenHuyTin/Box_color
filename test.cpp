...
int main(int argc, char* argv[]) {
    ...
    // Load a font
    TTF_Font* font = TTF_OpenFont("OpenSans-Light.ttf", 14);
    if (!font) {
...
// SDL_RenderDrawRoundRect(renderer, rect, cornerRadius);
// SDL_RenderFillRoundRect(renderer, rect, cornerRadius);
...
//-------------------------------------------------------------------------Draw squares in first rectangle-------------------------------------------------
    ...
    for (int row = 0; row < 3; row++) {
        ...
        for (int col = 0; col < 5; col++) {
            ...
            SDL_Surface* surface = TTF_RenderText_Solid(font, "0", textColor);;
            if(r == 253 && g==136 && b == 81 && a == 255)
            {
                surface = TTF_RenderText_Solid(font, "1", textColor);
            }
            else if(r == 153 && g==153 && b == 153 && a == 255)
            {
                surface = TTF_RenderText_Solid(font, "2", textColor);
            }
            else if(r == 255 && g==191 && b == 40 && a == 255)
            {
                surface = TTF_RenderText_Solid(font, "3", textColor);
            }
            else if(r == 255 && g==191 && b == 41 && a == 255)
            {
                surface = TTF_RenderText_Solid(font, "4", textColor);
            }
            else if(r == 76 && g==133 && b == 217 && a == 255)
            {
                surface = TTF_RenderText_Solid(font, "5", textColor);
            }
            else if(r == 66 && g==193 && b == 129 && a == 255)
            {
                surface = TTF_RenderText_Solid(font, "6", textColor);
            }
            else if(r == 138 && g==207 && b == 87 && a == 255)
            {
                surface = TTF_RenderText_Solid(font, "7", textColor);
            }
            else if(r == 0 && g == 174 && b == 238 && a == 255)
            {
                surface = TTF_RenderText_Solid(font, "8", textColor);
            }
            else if(r == 180 && g == 180 && b == 180 && a == 255)
            {
                surface = TTF_RenderText_Solid(font, "9", textColor);
            }
...
//------------------------------------------------------------Button apply-----------------------------------------------------------------------------
    ...
    // Set the text color
    SDL_Color textColor2 = { 0, 0, 0 };  // White color
...
//------------------------------------------------------------------Button Cancel--------------------------------------------------------------------
    ...
    // SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
    // SDL_RenderFillRect(renderer, &buttonRect4);
    // SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
...
// Draw the rectangle
    ...
    square2.w = 23;
    square2.h = 33;
    square2.y = rect2.y + 8;   // Adjust the y-coordinate for the squares

    int colorIndex2 = 0;

    // Draw 3 rows of 5 squares
    for (int row = 0; row < 2; row++) {
        square2.y = rect2.y + 8 + (row * 40);  // Adjust the y-coordinate for each row
        for (int col = 0; col < 8; col++) {
            square2.x = rect2.x + 8 + (col * 34);  // Adjust the x-coordinate for each square
...
//-----------------------------------------------------------------draw double small rectangle 1-------------------------------------------------------------------
    ...
    SDL_Surface* imageSurface = IMG_Load("triangle.png");
    ...
    // Load the image surface from a file
    SDL_Surface* imageSurface2 = IMG_Load("triangle2.png");
...
//-----------------------------------------------------------------draw double small rectangle 2-------------------------------------------------------------------
    ...
    // // Create a texture from the image surface
    //SDL_Texture* imageTexture3 = SDL_CreateTextureFromSurface(renderer, imageSurface);
